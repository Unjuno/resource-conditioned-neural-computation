...
- `realtime_nn_price_cap_attn_mlp_subops_results.json` — three-seed decoder-like suboperation experiment: attention and MLP are separately skippable equal-cost work units; price reduces MLP work before attention propagation in all seeds, with hard-cap, dense-equivalent, price-blind, and loss-oracle audits.
...
- separate attention-vs-MLP suboperation composition changes under equal per-operation cost, not only prefix depth changes;
