...
For workflow run `32222225890`, the extracted source identity is:
...
Workflow run `32222225890`, job `95974722074`:
...
This removes an important modeling gap from the previous CBMC runtime-contract experiment. The previous proof directly linked the actual deadline-admission implementation but represented the budget/effective-work algebra separately. The new proof mechanically extracts the deployed neural-control functions themselves.

The current software evidence hierarchy is therefore:
...
neural numerical kernels
    -> abstracted for this control proof
exact compiled RV32 binary
    -> separate taint/noninterference audit
pinned Ibex RTL
    -> separate measured timing binding
```
...
3. formal correctness of the custom RV32 taint interpreter;
