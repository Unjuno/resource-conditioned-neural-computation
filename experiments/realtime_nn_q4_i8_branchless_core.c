...
/*
 * Branchless symmetric round-to-nearest for Q4. Inputs are statically bounded
 * far away from INT32_MIN, so two's-complement magnitude conversion is safe.
 */
...
/* Branchless clamp to the direct Q4 LUT domain [-128, 128]. */
static inline int32_t clamp_lut_index(int32_t x) {
    int32_t from_lo = x - RTNN_Q_LUT_LO;
    uint32_t below = (uint32_t)from_lo >> 31;
    uint32_t lo_mask = 0u - below;
    uint32_t ux = ((uint32_t)x & ~lo_mask) | ((uint32_t)RTNN_Q_LUT_LO & lo_mask);
    x = (int32_t)ux;

    int32_t to_hi = RTNN_Q_LUT_HI - x;
    uint32_t above = (uint32_t)to_hi >> 31;
    uint32_t hi_mask = 0u - above;
    ux = ((uint32_t)x & ~hi_mask) | ((uint32_t)RTNN_Q_LUT_HI & hi_mask);
    return (int32_t)ux;
}
