...
static inline float gelu(float x){return .5f*x*(1.f+erff(x*.7071067811865475f));}
...
static void run_block(Scratch*s,int bi){
    ...
    for(int p=0;p<L;p++){
        ...
        for(int j=0;j<C;j++)s->z[p][j]=tanhf(a[j]+n[j]);
        lin(q->f1w,q->f1b,s->z[p],s->tmp[p],F,C);for(int j=0;j<F;j++)s->tmp[p][j]=gelu(s->tmp[p][j]);
        lin(q->f2w,q->f2b,s->tmp[p],s->ff[p],C,F);for(int j=0;j<C;j++)s->h[p][j]=tanhf(s->z[p][j]+.2f*s->ff[p][j]);
    }
}
static Result finish(const Scratch*s){float y[2];lin(M.hw,M.hb,s->h[0],y,2,C);Result r={y[0],y[1]};return r;}

// Explicit finite execution classes. A static analyzer can inspect each call graph
// without relying on a runtime loop bound for the number of optional blocks.
...
int main(int argc,char**argv){
    if(!init_model())return 3;
    int reps=argc>1?atoi(argv[1]):2000;if(reps>20000)reps=20000;
