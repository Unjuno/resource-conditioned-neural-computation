...
def main():
    ...
    tr,va,te=r.data(); m=r.train(a.seed,*tr,a.steps); tau,_,_,max_exit=r.tune(m,*va)
    tau_q=int(round(tau*S))
    ...
    with out.open('w') as f:
        ...
        xs=np.linspace(-32.,0.,8193); write_arr(f,'fx_exp_lut',np.rint(np.exp(xs)*PS).astype(np.int64),'uint32_t')
        xs=np.linspace(-8.,8.,4097); gel=.5*xs*(1+np.vectorize(math.erf)(xs/math.sqrt(2.))); write_arr(f,'fx_gelu_lut',np.rint(gel*S).astype(np.int64))
        f.write('#endif\n')
    meta={'seed':a.seed,'q':Q,'probability_q':PQ,'float_tau':tau,'tau_q':tau_q,'tau_q_real':tau_q/S,'max_exit':max_exit,'generated_header_sha256':hashlib.sha256(out.read_bytes()).hexdigest()}
